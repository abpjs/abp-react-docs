...
```bash
npm update @abpjs/core @abpjs/theme-basic @abpjs/theme-shared
```
...
2. **Update ChangePassword and Profile imports** (recommended): Change imports from `@abpjs/theme-basic` to `@abpjs/theme-shared`.

```bash
# Find usages
grep -rE "import.*ChangePassword.*theme-basic|import.*Profile.*theme-basic" src/
```
